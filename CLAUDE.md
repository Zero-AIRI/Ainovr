...
| `stream-route.ts` | `src/lib/api/` | API route 工厂，`createStreamRoute()` 消除 43 个 route 的重复样板 |
...
Prompt 构建器在 `src/lib/ai/prompts/`，每个导出 `build*Messages()` + `DEFAULT_SYSTEM_PROMPT`。统一从 `src/lib/ai/prompts/index.ts` 导出。

Prompt 注册表在 `src/lib/store/prompts.ts`（`PROMPT_REGISTRY`），分为四个类别：统一管线、素材处理、层级生成、章节写作。用户可通过 PromptManagementView 自定义任意 prompt，覆盖存储在服务端 `data/prompts.json`。
...
Route Handler 按领域分组：
- `source/process/` — 统一管线步骤（event-extraction, event-alignment, deep-analysis, summary, dna-compression）+ 旧版步骤（slice, style, plot 等逐步废弃）
...
多数 source/process/* 和 generation/* 路由使用 `createStreamRoute()` 一行创建，模式统一。
...
| `src/lib/store/prompts.ts` | Prompt 注册表 + 自定义覆盖 store |
| `src/lib/source-processing/unified-pipeline.ts` | 统一 7 步管线编排器 |
| `src/lib/source-processing/pipeline-types.ts` | 管线类型定义（SliceAnalysis, SummaryReport, GenerationRulesDNA 等） |
