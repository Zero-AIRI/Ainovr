// ============================================
// POST /api/generation/chapter-plan-framework — 每章计划框架生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { setContent, minorPlotPatterns, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildChapterPlanFrameworkMessages(setContent, minorPlotPatterns);
    ...
    const message = error instanceof Error ? error.message : '每章计划框架生成失败';
