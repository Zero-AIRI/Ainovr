// ============================================
// POST /api/generation/phase-detail — 阶段详细生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { outline, phaseFramework, phaseIndex, plotGuide, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildPhaseDetailMessages(outline, phaseFramework, phaseIndex, plotGuide);
    ...
    const message = error instanceof Error ? error.message : '阶段详细生成失败';
