// ============================================
// POST /api/generation/chapter-set-framework — 章节集合框架生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { volumeContent, minorPlotPatterns, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildChapterSetFrameworkMessages(volumeContent, minorPlotPatterns);
    ...
    const message = error instanceof Error ? error.message : '章节集合框架生成失败';
