// ============================================
// POST /api/generation/volume-detail — 分卷详细生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { outline, phaseContent, volumeFramework, volumeIndex, minorPlotPatterns, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildVolumeDetailMessages(outline, phaseContent, volumeFramework, volumeIndex, minorPlotPatterns);
    ...
    const message = error instanceof Error ? error.message : '分卷详细生成失败';
