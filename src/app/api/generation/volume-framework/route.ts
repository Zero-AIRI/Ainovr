// ============================================
// POST /api/generation/volume-framework — 分卷框架生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { outline, phaseContent, minorPlotPatterns, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildVolumeFrameworkMessages(outline, phaseContent, minorPlotPatterns);
    ...
    const message = error instanceof Error ? error.message : '分卷框架生成失败';
