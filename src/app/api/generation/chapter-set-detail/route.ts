// ============================================
// POST /api/generation/chapter-set-detail — 章节集合详细生成
// ============================================
...
export async function POST(req: Request) {
  try {
    ...
    const { volumeContent, setFramework, setIndex, minorPlotPatterns, apiKey, model, baseURL } = body;
    if (!apiKey) return new Response(JSON.stringify({ error: '请先配置 API Key' }), { status: 400 });
    const { systemPrompt, userMessage } = buildChapterSetDetailMessages(volumeContent, setFramework, setIndex, minorPlotPatterns);
    ...
    const message = error instanceof Error ? error.message : '章节集合详细生成失败';
